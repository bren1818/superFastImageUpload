				...
				<?php
					...
					function foldersize($dir){
						...
						if ($handle = opendir('uploads/xhr/'.$sessionID)) {
							...
							$cwd = $url.'/bUpload/uploads/xhr/'.$sessionID."/";
							
							$sample = foldersize(getcwd().'/uploads/xhr/'.$sessionID."/" );
							echo "<p>File Count : " . $sample[1] . " Files <br/>" ;
							echo "Folder Size : " . (($sample[0] /1024) /1024) . " MB</p>" ;
							...
							while (false !== ($entry = readdir($handle))) {
								if ($entry != "." && $entry != "..") {
									//if unix
									$img = getcwd() ."/uploads/xhr/".$sessionID."/".$entry; //unix slashes /
									//if windows
									//$img = getcwd() ."\\uploads\\xhr\\".$sessionID."\\".$entry; //windows slashes
